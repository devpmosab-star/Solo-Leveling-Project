...
export default function App() {
  ...
  function showDamage(value, type = 'damage') {
  ...
  function useSkill(skill) {
    ...
    showDamage(current.damage, 'skill')
  ...
  return (
    <main className={"app " + (
      ...
      <div className="damage-layer">
        {damageNumbers.map(d => (
          <b className={'damage-number ' + d.type} key={d.id}>-{d.value}</b>
        ))}
      </div>
      ...
      {tab === 'combat' && <CombatArena activeDungeon={activeDungeon} spawnDungeon={spawnDungeon} attackDungeon={attackDungeon} useSkill={useSkill} playerHp={playerHp} mana={mana} dungeonLog={dungeonLog} />}
...
function CombatArena({ activeDungeon, spawnDungeon, attackDungeon, useSkill, playerHp, mana, dungeonLog }) {
  const hpPercent = activeDungeon ? Math.max(0, Math.round((activeDungeon.currentHp / activeDungeon.hp) * 100)) : 0

  return (
    <section className="combat-arena">
      <div className="arena-bg">
        <div className="arena-moon"></div>
      ...
      <div className="hunter-side">
        ...
        <div className="combat-bars">
          <div className="xpRow"><b>HP</b><b>{playerHp}</b></div>
          <div className="bar hp-bar"><div style={{ width: '100%' }} /></div>
          <div className="xpRow"><b>MANA</b><b>{mana}</b></div>
          <div className="bar mana-bar"><div style={{ width: `${Math.min(100, mana / 3)}%` }} /></div>
        </div>
      </div>
      ...
      <div className="boss-side">
        {!activeDungeon ? (
          <div className="no-boss">
            <h2>No Boss Detected</h2>
            <p>Open a gate to begin cinematic combat.</p>
            <button onClick={spawnDungeon}>Open Gate</button>
          </div>
        ) : (
          <>
            ...
            <h3>{activeDungeon.boss}</h3>
            <p>{activeDungeon.name} · Rank {activeDungeon.rank}</p>
            <div className="combat-bars">
              <div className="xpRow"><b>BOSS HP</b><b>{activeDungeon.currentHp}/{activeDungeon.hp}</b></div>
              <div className="bar danger-bar"><div style={{ width: `${hpPercent}%` }} /></div>
            </div>
            <div className="skill-console">
              ...
              <button onClick={() => useSkill('Shadow Slash')}>Shadow Slash</button>
              <button onClick={() => useSkill('Monarch Authority')}>Monarch Authority</button>
              <button onClick={() => useSkill('Dagger Rush')}>Dagger Rush</button>
            </div>
          </>
        )}
      </div>
      ...
      <div className="combat-log-panel">
        <h3>Combat Log</h3>
        {dungeonLog.slice(0, 5).map((l, i) => <p key={i}>› {l}</p>)}
      </div>
    </section>
  )
}
